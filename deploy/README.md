...
The live demo node serves the landing at **https://metis.modelmarket.dev** and
proxies the Metis API behind it. This folder holds the reproducible config.
...
Both containers share the `metisnet` docker network, so nginx reaches the API
by its container name (`metis`). Only 80/443 are published to the host.
