...
def create_app(config: Optional[RuntimeConfig] = None) -> FastAPI:
    ...
    @app.get("/metrics")
    async def metrics():
        """Prometheus exposition (text/plain 0.0.4). Scrape target for Grafana/alerting —
        `up`/scrape-failure catches a down service; breaker gauges catch provider outages."""
        from starlette.responses import PlainTextResponse
        lines: list[str] = [
            "# HELP metis_up 1 if the service is serving requests",
            "# TYPE metis_up gauge",
            "metis_up 1",
            "# HELP metis_build_info build/version info",
            "# TYPE metis_build_info gauge",
            'metis_build_info{version="0.2.0"} 1',
        ]
        kn = brain._knowledge_store.count() if brain._knowledge_store else 0
        lines += ["# HELP metis_knowledge_entries entries in the knowledge store",
                  "# TYPE metis_knowledge_entries gauge", f"metis_knowledge_entries {kn}"]
        lines += ["# HELP metis_circuit_breaker_open 1 when a provider circuit breaker is open",
                  "# TYPE metis_circuit_breaker_open gauge"]
        for b in all_breaker_status():
            st = 1 if str(b.get("state")).lower() == "open" else 0
            lines.append(f'metis_circuit_breaker_open{{endpoint="{b.get("endpoint","?")}"}} {st}')
            lines.append(f'metis_circuit_breaker_failures{{endpoint="{b.get("endpoint","?")}"}} {b.get("failure_count",0)}')
        ts = getattr(brain, "_trace_store", None)
        if ts is not None:
            for fn in ("aggregate_stats", "stats", "summary"):
                agg = getattr(ts, fn, None)
                if callable(agg):
                    try:
                        s = agg() or {}
                        if isinstance(s, dict):
                            lines += ["# TYPE metis_traced_runs_total counter",
                                      f"metis_traced_runs_total {int(s.get('total', s.get('runs', 0)) or 0)}",
                                      "# TYPE metis_traced_failures_total counter",
                                      f"metis_traced_failures_total {int(s.get('failures', 0) or 0)}"]
                    except Exception:
                        pass
                    break
